...
There are \textit{objective data} and the "subjective" data or \textit{metadata}.
...
In whis work we propose a method that allows for clustering dataset with \textit{objective data}, and explain differences between clusters with \textit{metadata}.
...
In particular it is not possible to provide additional \textit{metadata} for explanations, nor modify the set of conccepts that are used for explanations.
...
Moreover, there are presented example observations on Figure~\ref{fig:example_products}, word clouds describing clusters on Figure~\ref{fig:wordcloud} and LIME explanation for one instance of \textit{metadata} Figure~\ref{fig:lime}.
